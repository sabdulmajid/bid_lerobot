...
def bidirectional_sampler(strong, weak, prior, obs_dict, ah_count, ah_test, temperature=1.0, num_sample=40, beta=1.0, factor=2):
    """
    ...
    if prior is not None:
        ...
        # dist_strong = manhattan_distance(action_strong_batch['action_pred'][:, :, :CH], prior.unsqueeze(1), reduction='none')
        dist_strong = cosine_distance(action_strong_batch['action_pred'][:, :, :CH], prior.unsqueeze(1), reduction='none')
        ...
        # dist_weak = manhattan_distance(action_weak_batch['action_pred'][:, :, :CH], prior.unsqueeze(1), reduction='none')
        dist_weak = cosine_distance(action_weak_batch['action_pred'][:, :, :CH], prior.unsqueeze(1), reduction='none')
    ...
    tar_expand =  action_strong_batch['action_pred'].unsqueeze(2)
    # dist_pos = euclidean_distance(src_expand, tar_expand).view(B, num_sample, num_sample)
    # dist_pos = manhattan_distance(src_expand, tar_expand).view(B, num_sample, num_sample)
    dist_pos = cosine_distance(src_expand, tar_expand).view(B, num_sample, num_sample)
    ...
    tar_expand = action_weak_batch['action_pred'].unsqueeze(2)
    # dist_neg = euclidean_distance(src_expand, tar_expand).view(B, num_sample, num_sample)
    # dist_neg = manhattan_distance(src_expand, tar_expand).view(B, num_sample, num_sample)
    dist_neg = cosine_distance(src_expand, tar_expand).view(B, num_sample, num_sample)

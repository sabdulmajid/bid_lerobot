...
def _collect_mock_rollouts(
    ...
    advantages = _assign_step_advantages(
        ...
        quality_gate=cfg.get("polyppo", {}).get("quality_gate", "none"),
        quality_quantile=float(cfg.get("polyppo", {}).get("quality_quantile", 0.5)),
        lambda_bad=float(cfg.get("polyppo", {}).get("lambda_bad", 0.0)),
        valid_mask=valid,
    )
